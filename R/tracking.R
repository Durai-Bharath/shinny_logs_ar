...
logAppOpen <- function(session, app_name, user) {
  log_info <- list(
    app = app_name,
    user = user,
    timestamp = Sys.time(),
...
saveLog <- function(log_info) {
  ...
  log_file <- file.path(log_folder, "log_file_sessision_start.log")
  # log_file <- "path/to/log_file.log" # Make sure this path is writable
  write(jsonlite::toJSON(log_info, auto_unbox = TRUE), log_file, append = TRUE)
}
...
track_usage <- function(storage_mode,
  ...
  logAppOpen(session, app_name, user)
